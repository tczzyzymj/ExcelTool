namespace ExcelTool
{
    public abstract class CommonWorkSheetData
    {
        ...
        ///     缓存的数据
        /// </summary>
        private readonly Dictionary<string, List<CellValueData>> mCacheDataMap =
        new Dictionary<string, List<CellValueData>>();
        ...
        protected bool mHasLoadAllCellData;
        protected WeakReference<FileDataBase>? mOwnerTable;

        public int IndexInListForShow { get; protected set; }
        ...
        public virtual List<CellValueData>? GetRowCellDataByTargetKeysAndValues(List<int> matchKeyList,
            List<string>                                                                  matchValueList)
        {
            ...
            {
                string _msg =
                $"{GetRowCellDataByTargetKeysAndValues}, mCellData2DList 为空，sheet：{DisplayName},表格：{GetOwnerTable()?.GetFileName(false)} ";
            ...
            {
                throw new
                Exception("GetListCellDataBySpecificKeysAndValues, 传入的 List<CellValueData> matchValueList 数据为空");
            }

            if (matchValueList.Count != matchKeyList.Count)
            {
            ...
            {
                List<CellValueData> _rowData = mCellData2DList[_rowIndex];

                int _matchCount = 0;

                for (int i = 0; i < matchKeyList.Count; ++i)
                {
                    int _matchKeyIndex = matchKeyList[i];
                    ...
                    string _matchKeyCellValue = _rowData[_matchKeyIndex].GetCellValue().Trim();

                    foreach (string _matchTargetStr in matchValueList)
                    {
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public virtual List<string> GetRowStringDataByTargetKeysAndValus(
        List<int>    matchKeyList,
        List<string> matchValueList)
        {
            ...
            List<CellValueData>? _rowData = GetRowCellDataByTargetKeysAndValues(matchKeyList, matchValueList);

            if (_rowData != null)
            {
                foreach (CellValueData _singleData in _rowData)
                {
        ...
        public virtual bool WriteOneData(int rowIndexInSheet, List<string> inValueList, bool skipEmptyData)
        {
            ...
            List<KeyData>? _keyList = GetKeyListData();
            ...
            {
                throw new Exception($"错误，_keyList.Count : {_keyList.Count}, inValueList.Count : {inValueList.Count}");
            }
        ...
        {
            if ((mOwnerTable == null) || !mOwnerTable.TryGetTarget(out FileDataBase? _result))
            {
                return null;
            }
        ...
        // 注意，这里只初始化 Key
        public bool Init(WeakReference<FileDataBase> ownerExcelFile,
                         object                      sheetData,
                         int                         indexInListValue,
                         int                         indexInFileData,
                         string                      name)
        {
        ...
        public void CacheDataByMainKey(bool isForceLoad)
        {
            ...
            List<List<CellValueData>>? _allDataList = GetAllDataList();
            ...
            List<KeyData> _keyDataList = GetKeyListData();
            int           _keyIndex    = -1;
            ...
            {
                string _keyStr = _allDataList[i][_keyIndex].GetCellValue();
        ...
        {
            mCacheDataMap.TryGetValue(keyStr, out List<CellValueData>? cellValueDataList);
        ...
        {
            KeyData? _existData = mKeyDataList.Find(x => x.KeyIndexInList == indexInList);
